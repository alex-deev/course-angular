...
@Component({
  ...
  template: `
    <div id="nav" class="w-full px-6">
      @for (item of menuOption; track item.route) {
        <a
      ...
      @for (searchKey of gifsService.searchHistoryKeys(); track $index) {
        <a
          [routerLink]="['/dashboard/history', searchKey]"
          routerLinkActive="bg-blue-800"
          class="w-full px-2 inline-flex space-x-2 items-center border-b border-slate-700 py-3 hover:bg-white/5 transition ease-linear duration-150"
        >
          <div>
          ...
          <div class="flex flex-col">
            <span class="text-lg font-bold leading-5 text-gray-400">{{ searchKey }}</span>
          </div>
        </a>
      }
    </div>
